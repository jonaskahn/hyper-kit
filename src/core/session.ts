...
const OSC0_PATTERN = /\x1b\]0;([^\x07\x1b]*?)(?:\x07|\x1b\\)/g;
...
/* OSC 0 window titles: shells emit them on preexec (the running command) and
   precmd (back at the prompt). Last title in the chunk wins. */
export function parseOsc0(data: string): string | null {
  let match: RegExpExecArray | null;
  let title: string | null = null;
  while ((match = OSC0_PATTERN.exec(data))) {
    title = match[1];
  }
  return title;
}
